...
def encontrar_jobs(request):
    if request.method == 'GET':
        ...
        if not prazo_minimo:
            prazo_minimo = datetime(year=2000, month=1, day=1)

        if not prazo_maximo:
            prazo_maximo = datetime(year=2050, month=12, day=1)
...
def perfil(request):
    ...
    if request.method == 'GET':
        ...
        # verifica se já existe um usuário com esse username que não seja eu
